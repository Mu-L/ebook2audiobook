...
**Hãy xem repo khác của chúng tôi dành riêng cho việc thêm SML tự động vào sách điện tử của bạn -> [E2A-SML](./tools/E2A-SML)**
...
2. **Cài đặt / Chạy ebook2audiobook**:
   ...
   - **Linux/MacOS**  
     ...
     ```
     <i>Lưu ý cho người dùng macOS: homebrew được cài đặt để cài các chương trình còn thiếu.</i>
   ...
   - **Windows**  
     ...
     ```
     hoặc
     Nhấp đúp vào `ebook2audiobook.cmd`
...
###  Ví dụ về việc tải lên Mô hình tùy chỉnh dạng Zip
  (phải là tệp .zip chứa các tệp mô hình bắt buộc. Ví dụ cho XTTSv2: config.json, model.pth, vocab.json và ref.wav)
   - **Linux/MacOS**
     ```bash
   ...
   - **Windows**
     ...
     ```
     <i>Lưu ý: ref.wav của mô hình tùy chỉnh của bạn luôn là giọng được chọn để chuyển đổi</i>
     
- **<custom_model_path>**: Đường dẫn đến tệp `model_name.zip`,
      tệp này phải chứa (tùy theo engine tts) tất cả các tệp bắt buộc<br>
      (xem ./lib/models.py).

### Để có Hướng dẫn chi tiết với danh sách tất cả các Tham số cần dùng
   - **Linux/MacOS**
     ```bash
   ...
   - **Windows**
     ...
     ```
   - **Hoặc cho tất cả các hệ điều hành**
    ```python
     app.py --help
    ```
...
```
- LƯU Ý: MPS không được hiển thị trong Docker nên phải dùng CPU
...
Các bản phát hành có thể được tìm thấy -> [tại đây](https://github.com/DrewThomasson/ebook2audiobook/releases)
```bash
