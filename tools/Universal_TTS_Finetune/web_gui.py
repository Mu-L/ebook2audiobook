    ...
    with gr.Blocks(title="Universal TTS Finetune", theme=theme, css=css_str) as demo:
        ...
        with gr.Tab("1 - Prepare dataset"):
            ...
            gr.Markdown(
                ...
                "- **Plain Text (.txt) - Forced Alignment**: Upload a plain text book file (e.g. converted from ePUB using Calibre) along with a single full audiobook file to run **Forced Alignment**. **IMPORTANT**: The text file must be formatted with **one sentence per line** so the aligner knows where to slice. You can easily do this in any text editor by search-replacing the regex `(?<=[.!?])\\s+` with a newline `\\n`.\n"
