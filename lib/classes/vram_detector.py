...
class VRAMDetector:
	...
	def _get_windows_vram(self):
		...
		for block in out.split("\n\n"):
			...
			if name:
				...
				gpus.append({"name": name.group(1).strip(),
				             "vram_bytes": val,
				             "vram": self._fmt(val)})
		if any(g["vram_bytes"]>0 for g in gpus):
			return gpus

		# quick dxdiag fallback (timeout-safe)
		with tempfile.NamedTemporaryFile(delete=False, suffix=".txt") as tf:
			path = tf.name
		try:
			subprocess.Popen(["dxdiag","/t",path],
	...
	# ---- main API ----
	def detect_vram(self,as_json=False):
		sys=self.system
		if sys=="windows":
