        ...
        with gr.Blocks(theme=theme, title=title, css=header_css, delete_cache=(604800, 86400)) as app:
                        ...
                        with gr.Row(elem_id='gr_row_tab_main'):
                            with gr.Column(elem_id='gr_col_1', elem_classes=['gr-col'], scale=3):
                                with gr.Group(elem_id='gr_group_ebook_file', elem_classes=['gr-group']):
                                    ...
                                    gr_ebook_textarea = gr.Textbox(label=ebook_mode_labels['text'], elem_id='gr_ebook_textarea', value='', visible=False, show_label=True, lines=7, max_lines=6, max_length=max_ebook_textarea_length)
                ...
                try:
                    ...
                    if session and session.get('id', False):
                        ...
                        if val == 'single':
                            ...
                            return gr.update(visible=True, label=ebook_mode_labels['directory'], value=None, file_count='directory'), gr.update(visible=False, value=None)
                        elif val == 'text':
            ...
            def change_gr_restore_session(data:DictProxy|None, state:dict, req:gr.Request)->tuple:
                try:
                    ...
                    print(f"ebook_file: {data.get('ebook_file')}")
                    print(f"ebook_textarea: {data.get('ebook_textarea')}")
