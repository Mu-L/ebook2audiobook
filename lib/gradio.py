...
def build_interface(args:dict)->gr.Blocks:
    ...
    try:
        ...
        with gr.Blocks(theme=theme, title=title, css=header_css, delete_cache=(604800, 86400)) as app:
                        ...
                        with gr.Row(elem_id='gr_row_tab_main'):
                            with gr.Column(elem_id='gr_col_1', elem_classes=['gr-col'], scale=3):
                                with gr.Group(elem_id='gr_group_ebook_file', elem_classes=['gr-group']):
                                    ...
                                    gr_ebook_clear_btn = gr.ClearButton(gr_ebook_textarea, elem_id='#gr_ebook_clear_btn', value='🗑️', size='sm')
                                    gr_ebook_textarea = gr.Textbox(label=ebook_mode_labels['text'], elem_id='gr_ebook_textarea', variant='secondary', value='', visible=False, show_label=True, lines=6, max_lines=6, max_length=max_ebook_textarea_length, info=f'0 / {max_ebook_textarea_length}') #  buttons=[gr_ebook_clear_btn] > gradio v6+
                ...
                try:
                    ...
                    if session and session.get('id', False):
                        ...
                        args = {
                            ...
                            "ebook": None,
                            "ebook_src": ebook_file if isinstance(ebook_file, str) else None,
            ...
            def check_override_audiobook(session_id:str, ebook_data:any, ebook_textarea:str, blocks_preview:bool, event:int)->tuple:
                session = context.get_session(session_id)
                source = None
                if session and session.get('id', False):
