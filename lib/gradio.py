    ...
    try:
        ...
        with gr.Blocks(theme=theme, title=title, css=header_css, delete_cache=(604800, 86400)) as app:
            ...
            with gr_blocks_group:
                ...
                with gr_row_buttons:
                    gr.Column(scale=1)
                    gr_blocks_cancel = gr.Button('✖', elem_classes=['confirm-buttons'], variant='stop', scale=0, min_width=180)
                    gr.Column(scale=0, min_width=100)
                    gr_blocks_continue = gr.Button('✔', elem_classes=['confirm-buttons'], variant='primary', scale=0, min_width=180)
                    gr.Column(scale=1)
