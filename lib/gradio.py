        ...
        with gr.Blocks(theme=theme, title=title, css=header_css, delete_cache=(604800, 86400)) as app:
            ...
            with gr.Group(visible=False, elem_id='gr_group_blocks', elem_classes='gr-group-main') as gr_group_blocks:
                ...
                with gr.Row(elem_id='gr_row_buttons', visible=True) as gr_row_buttons:
                    gr.Column(scale=1)
                    gr_blocks_cancel = gr.Button('✖', elem_classes=['blocks-buttons'], variant='stop', scale=0, size='sm',  min_width=)
                    gr.Column(scale=0, min_width=100)
                    gr_blocks_continue = gr.Button('✔', elem_classes=['blocks-buttons'], variant='primary', scale=0, size='sm',  min_width=)
                    gr.Column(scale=1)
