    ...
    def export_audio(ffmpeg_combined_audio, ffmpeg_metadata_file, ffmpeg_final_file):
        try:
            ...
            if session['output_format'] == 'wav':
                ffmpeg_cmd += ['-map', '0:a', '-ar', '44100', '-sample_fmt', 's16', '-c:a', 'pcm_s16le']
            elif session['output_format'] == 'aac':
                ffmpeg_cmd += ['-c:a', 'aac', '-b:a', '128k', '-ar', '44100']
            elif session['output_format'] == 'flac':
                ffmpeg_cmd += ['-c:a', 'flac', '-compression_level', '3', '-ar', '44100', '-sample_fmt', 's16']
            else:
                ...
                if session['output_format'] in ['m4a', 'm4b', 'mp4', 'mov']:
                    ...
                    ffmpeg_cmd += ['-c:a', 'libshine', '-b:a', '128k', '-ar', '44100']
                elif session['output_format'] == 'webm':
                    ffmpeg_cmd += ['-c:a', 'libopus', '-b:a', '96k', '-ar', '48000', '-compression_level', '0']
                elif session['output_format'] == 'ogg':
                    ffmpeg_cmd += ['-c:a', 'libopus', '-b:a', '96k', '-ar', '48000', '-compression_level', '0']
                ffmpeg_cmd += ['-map_metadata', '1']
            if session.get('compliance') == 'ebu_r128':
                ffmpeg_cmd += [
                ...
                ffmpeg_cmd += [
                    '-af', 'highpass=f=80,lowpass=f=12000,afftdn=nf=-60,dynaudnorm=f=75:g=15,acompressor=threshold=-16dB:ratio=3:attack=10:release=200',
                    '-sample_fmt', 's16',
                    '-threads', str(os.cpu_count()),
                    '-progress', 'pipe:1',
                    '-y', ffmpeg_final_file
