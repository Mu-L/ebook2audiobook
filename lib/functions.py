    ...
    with gr.Blocks(theme=theme, title=title, css=header_css, delete_cache=(86400, 86400)) as app:
        with gr.Group(visible=True, elem_id='gr_group_main', elem_classes='gr-group-main') as gr_group_main:
            ...
            with gr_group_audiobook_list:
                ...
                gr_audiobook_files = gr.Files(label="Downloads", elem_id='gr_audiobook_files', visible='hidden')
            ...
            try:
                ...
                if audiobook is not None: 
                    ...
                    if not os.path.exists(audiobook) or not os.path.exists(vtt):
                        return gr.update(visible=True if len(audiobook_options) > 0 else 'hidden'), gr.update(value=None), gr.update(value=None)
                    ...
                    with open(vtt, "r", encoding="utf-8-sig", errors="replace") as f:
                        vtt_content = f.read()
                    return gr.update(visible=True if len(audiobook_options) > 0 else 'hidden'), gr.update(value=audiobook), gr.update(value=vtt_content)
            except Exception as e:
